...
CMD:aderby(playerid, params[])
{
	...
	switch(DI[STATUS])
	{
		case DERBY_CLOSED:
		{
			...
			SendClientMessageToAll(-1, "{FFFFFF}<< DERBY >> {45D0FF}En 30 segundos el {FFFFFF}Derby {45D0FF}comenzará! Escribe {FFFFFF}/aderby {45D0FF}para participar");
            SendClientMessage(playerid,COLOR_ROJO,"{FFFFFF}Si quieres salir del minijuego derby usa {FF0015}/salird.");
...
public DerbyCountdown()
{
	...
	new str[145]; format(str, 145, "%d/%d_~g~~h~jugadores~n~~y~faltan_~w~%d_~y~segundos_para_empezar", DI[PLAYERS], MAX_DERBY_PLAYERS, DI[COUNTDOWN_COUNTER]);
...
PlayerDerbyDead(playerid)
{
	...
	new str[128]; format(str, 128, "", PI[playerid][P_NAME], DI[ACTIVE_PLAYERS], DI[PLAYERS], (DI[MAX_PRIZE] / DI[ACTIVE_PLAYERS]) );
